 ...
 */
@WebServlet("/listBoard.do")//url 맵핑설정
public class BoardListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int listCount;			//총게시물 갯수
		int currentPage;		//현재 페이지, (요청한 페이지)
		int startPage;			//현재 페이지 하단에 보여지는 페이지의 시작수
								//게시물의 수만큼 뿌려줘야 하는것을 알야하기때문에 끝페이지도 알아야한다.
		int endPage;			//현재 페이지 하단에 뵤여지는 페이징바의 끝수
		int maxPage;			//전체 페이지의 가장마지막페이지
		
		int pageLimit; 			//한페이지 하단에 보여질 페이지 최대갯수
		int boardLimit; 		// 한페이지에 뵤여질 게시글 최대갯수 
		...
		//페이지 전환시 전달받은 페이지가 있을 경우 전달받은 페이지를 currentPage 에 담기
		if(request.getParameter("currentPage") != null) {
			currentPage = Integer.parseInt(request.getParameter("currentPage"));//int 이기때문에 병형
		}
		...
		*/
		...
		//단, 예를 들어 maxPage가 13이면 endPage 도 13
		if(maxPage < endPage) {
			endPage = maxPage;
		}
